...
import Util
from LyricsSearch import LyricsSearcher
...
from DownAllWindow import DownAllWindow
...
class lLyrics(GObject.Object, Peas.Activatable):
    ...
    def do_activate(self):
        ...
        self.skc_id = self.settings.connect('changed',
                                            self.get_user_preferences, config)
        ...
        if self.player.props.playing:
                self.search_lyrics(self.player,
                                   self.player.get_playing_entry())
        # Search lyrics everytime the song changes
        self.psc_id = self.player.connect('playing-song-changed',
                                          self.search_lyrics)
        # Connect to elapsed-changed signal to handle synchronized lyrics
        self.pec_id = self.player.connect('elapsed-changed',
    ...
    def get_user_preferences(self, settings, key, config):
        ...
        # if this is called in do_activate or we need
        # a reload to apply, return here
    ...
    def insert_ui(self):
        self.appshell.add_app_menuitems(view_menu_ui,
                                        ...
                                        'view')
        self.appshell.add_browser_menuitems(context_ui,
    ...
    def init_menu(self):
        ...
        self.toggle_action_group.add_action(func=self.toggle_visibility,
            ...
            label=_("Lyrics"), action_state=ActionGroup.TOGGLE,
    ...
    def init_sidebar(self):
        ...
        pxbf = GdkPixbuf.Pixbuf.new_from_file(os.path.dirname(__file__)
                                              + "/menu-arrow.png")
        icon_factory.add("llyrics_menu",
                         Gtk.IconSet.new_from_pixbuf(pxbf))
        icon_factory.add_default()

        menu_button = Gtk.Image.new_from_stock("llyrics_menu",
        ...
        self.tag = self.textbuffer.create_tag(None,
    ...
    def get_button_menu(self):
        ...
        item_unselect.connect("activate",
                              self.scan_selected_source_callback,
                              "SelectNothing")
        ...
        last_item = self.add_radio_menu_item(
                                        ...
                                        last_item)
        self.radio_sources.append(Gtk.SeparatorMenuItem())
        self.add_radio_menu_item(self.radio_sources, _("From cache file"),
                                 self.scan_selected_source_callback, last_item)
        ...
        self.add_menu_item(menu, _("Scan next source"),
                           self.scan_next_action_callback)
        #self.add_menu_item(menu, _("Scan all sources"),
                                    #self.scan_all_action_callback)
        menu.append(Gtk.SeparatorMenuItem())
        self.add_menu_item(menu, _("Search online"),
                           self.search_online_action_callback)
        menu.append(Gtk.SeparatorMenuItem())
        self.add_menu_item(menu, _("Mark as instrumental"),
                           self.instrumental_action_callback)
        menu.append(Gtk.SeparatorMenuItem())
        self.add_menu_item(menu, _("Download lyrics for current playlist"),
                           self.download_all_lyrics_action_callback)
        menu.append(Gtk.SeparatorMenuItem())
        ...
        menu.append(Gtk.SeparatorMenuItem())
        self.add_menu_item(menu, _("Preferences"),
                           self.preferences_dialog_action_callback)
    ...
    def popup_menu(self, widget, event, menu):
        menu.popup(None, None, lambda x, y:
                   (event.x_root + event.x,
                    event.y_root + event.y, True),
    ...
    def search_lyrics(self, player, entry):
        ...
        if entry.get_entry_type().get_name() in ('iradio', 'podcast-post'):
            ...
            if not self.first:
                ...
                self.toggle_action_group.\
                        get_action("ToggleLyricSideBar").set_active(False)
            return
        ...
        if self.first and not self.showing_on_demand:
            ...
            if not self.visible and self.show_first:
                self.toggle_action_group.\
                        get_action("ToggleLyricSideBar").set_active(True)
                # toggling the sidebar will start lyrics search again,
        ...
        print("search lyrics for " + self.location + ' '
    ...
    def download_all_lyrics_action_callback(self, action):
        self.window = DownAllWindow()
        win = self.window.get_Down_All_Window()
        win.connect("delete-event", Gtk.main_quit)
        win.show_all()

        newthread = Thread(target=self.download_all_lyrics_thread)
        newthread.start()

    def download_all_lyrics_thread(self):
        ...
        for row in self.shell.props.library_source.props.query_model:
            ...
            if lyrics != "":
                ...
                if self.cache:
                    path = self.build_cache_path(artist, title)
            ...
            if self.window.is_canceled():
                ...
                break
        self.window.set_text(_("{0} found!").format(found_lyrics))
        self.window.change_button_text()
    ...
    def edit_action_callback(self, action):
        # Unset event flag to indicate editing and
        # so block all other threads which
        # want to display new lyrics until editing is finished.
        self.edit_event.clear()

        # Conserve lyrics in order to restore original
        # lyrics when editing is canceled
        start, end = self.textbuffer.get_bounds()
        self.lyrics_before_edit = self.textbuffer.get_text(start, end, False)
        # remove sync tag
        self.textbuffer.remove_tag(self.sync_tag, start, end)
        # Conserve cache path in order to be able to
        # correctly save edited lyrics although
    ...
    def preferences_dialog_action_callback(self, action):
        ...
        dialog = Gtk.Dialog(_('lLyrics Preferences'),
                            self.shell.get_property('window'),
    ...
    def context_action_callback(self, action, param=None, data=None):
        ...
        if not self.visible:
            self.toggle_action_group.get_action(
    ...
    def save_button_callback(self, button):
        ...
        # If playing song changed, set "searching lyrics..."
        # (might be overwritten immediately, if thread for
        # the new song already found lyrics)
    ...
    def scan_source(self, source, artist, title):
        ...
        newthread = Thread(target=self._scan_source_thread,
                           args=(source, artist, title))
        newthread.start()
    ...
    def start_search(self, location, artist, title):
        if self.edit_event.is_set():
            Gdk.threads_add_idle(GLib.PRIORITY_DEFAULT_IDLE,
                                 self.set_displayed_text,
                                 _("searching lyrics..."))

        newthread = Thread(target=self._start_search_thread,
                           args=(location, artist, title))
        newthread.start()
    ...
    def _start_search_thread(self, location, artist, title):
        ...
        if lyrics == "":
            ...
            if not self.was_corrected:
                ...
                (artist, title, corrected) = Util.\
                                                get_lastfm_correction(artist,
                                                                      title)
                if corrected:
                    (self.clean_artist, self.clean_title) =\
            ...
            Gdk.threads_add_idle(GLib.PRIORITY_DEFAULT_IDLE,
                                 self.set_radio_menu_item_active,
                                 "SelectNothing")
            self.current_source = None
        else:
            self.show_lyrics(lyrics)
    ...
    def get_lyrics_from_source(self, source, artist, title):
        ...
        Gdk.threads_add_idle(GLib.PRIORITY_DEFAULT_IDLE,
                             self.set_radio_menu_item_active, source)
